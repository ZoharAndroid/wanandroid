...
import com.zohar.wanandroid.presenter.wechat.WechatPresenter;
...
public class WechatFragment extends Fragment implements IWechatView {
    ...
    private List<SubKnowledge> mWechatList = new ArrayList<>();
    ...
    private void initViewPagerAndTabLayout() {
        mViewPager.setAdapter(new FragmentStatePagerAdapter(getFragmentManager()){
            ...
            public CharSequence getPageTitle(int position) {
                return  mWechatList.get(position).getName();
            }
        });
     ...
     * 装载多个列表的公众号文章页面
     */
    private void startWechatArticleListPager() {
        if (mWechatList == null) {
            return;
        }
    ...
    public void httpSuccess(Knowledge knowledge) {
        if (knowledge.getErrorCode() == 0){
            ...
            // 该id公众号下的文章的
            mWechatList.addAll(knowledge.getData());
            startWechatArticleListPager();
